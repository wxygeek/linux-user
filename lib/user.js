...
exports.getExpiration = function (username, callback){
  ...
  spawnWrapper('chage', [].concat(args_array, [username]), null, function(error, data){
    data = data.split('\n')

    data = {
      changed: new Date(data[0].split(': ')[1]),
      passwordExpires: data[1].split(': ')[1] === 'never' ? null : new Date(data[1].split(': ')[1]),
      inactive: data[2].split(': ')[1] === 'never' ? null : new Date(data[2].split(': ')[1]),
      accountExpires: data[3].split(': ')[1] === 'never' ? null : new Date(data[3].split(': ')[1]),
